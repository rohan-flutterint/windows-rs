...
impl IAudioFrameNativeFactory {
    ...
    {
        let mut result__ = ::std::ptr::null_mut();
        ::windows_core::vcall!(self.CreateFromMFSample(data.into_param().abi(), forcereadonly.into_param().abi(), &<T as ::windows_core::ComInterface>::IID, &mut result__)).from_abi(result__)
    }
}
...
impl IVideoFrameNativeFactory {
    ...
    {
        let mut result__ = ::std::ptr::null_mut();
        ::windows_core::vcall!(self.CreateFromMFSample(data.into_param().abi(), subtype, width, height, forcereadonly.into_param().abi(), ::core::mem::transmute(mindisplayaperture.unwrap_or(::std::ptr::null())), device.into_param().abi(), &<T as ::windows_core::ComInterface>::IID, &mut result__)).from_abi(result__)
    }
}
