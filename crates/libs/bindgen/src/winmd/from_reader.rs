    ...
    for item in reader.items() {
        ...
        for interface in metadata::type_def_interfaces(def, generics) {
            ...
            let reference = match &ty {
                Type::TypeName(type_name) if type_name.generics.is_empty() => writer.insert_type_ref(&type_name.namespace, &type_name.name),
                Type::TypeName(_) => writer.insert_type_spec(ty),
...
fn winmd_type(ty: &metadata::Type) -> Type {
    match ty {
        ...
        metadata::Type::TypeName(metadata::TypeName::GUID) => Type::GUID,
        metadata::Type::TypeName(metadata::TypeName::IUnknown) => Type::IUnknown,
        metadata::Type::IInspectable => Type::IInspectable,
        metadata::Type::TypeName(metadata::TypeName::HResult) => Type::HRESULT,
        metadata::Type::PSTR => Type::PSTR,
        metadata::Type::PWSTR => Type::PWSTR,
        metadata::Type::PCSTR => Type::PCSTR,
        metadata::Type::PCWSTR => Type::PCWSTR,
        metadata::Type::TypeName(metadata::TypeName::Type) => Type::Type,
        metadata::Type::TypeDef(def, generics) => Type::TypeName(TypeName { namespace: def.namespace().to_string(), name: def.name().to_string(), generics: generics.iter().map(winmd_type).collect() }),
