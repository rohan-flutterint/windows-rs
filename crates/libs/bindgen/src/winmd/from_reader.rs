    ...
    for item in reader.items() {
        ...
        for interface in metadata::type_def_interfaces(def, generics) {
            ...
            let reference = match &ty {
                Type::Name(type_name) if type_name.generics.is_empty() => writer.insert_type_ref(&type_name.namespace, &type_name.name),
                Type::Name(_) => writer.insert_type_spec(ty),
...
fn winmd_type(ty: &metadata::Type) -> Type {
    match ty {
        ...
        metadata::Type::TypeDef(def, generics) => Type::Name(TypeName { namespace: def.namespace().to_string(), name: def.name().to_string(), generics: generics.iter().map(winmd_type).collect() }),
