...
fn write_interface(writer: &mut winmd::Writer, namespace: &str, name: &str, member: &Interface) {
    ...
    for type_path in &member.extends {
        ...
        let reference = match &ty {
            winmd::Type::Name(type_name) if type_name.generics.is_empty() => writer.insert_type_ref(&type_name.namespace, &type_name.name),
            winmd::Type::Name(_) => writer.insert_type_spec(ty),
            rest => unimplemented!("{rest:?}"),
        };
...
fn write_class(writer: &mut winmd::Writer, namespace: &str, name: &str, member: &Class) {
    ...
    let extends = if let Some(base) = &member.base {
        match syn_type_path(namespace, &[], base) {
            winmd::Type::Name(base) => writer.insert_type_ref(&base.namespace, &base.name),
            rest => unimplemented!("{rest:?}"),
        }
    } else {
...
fn syn_path(namespace: &str, generics: &[String], path: &syn::Path) -> winmd::Type {
    ...
    winmd::Type::Name(winmd::TypeName { namespace: type_namespace, name: name.to_string(), generics: type_generics })
}
