...
pub fn writer(writer: &Writer, def: metadata::TypeDef) -> TokenStream {
    ...
    let has_unknown_base = matches!(vtables.first(), Some(metadata::Type::TypeName(metadata::TypeName::IUnknown)));
    ...
    match vtables.last() {
        Some(metadata::Type::TypeName(metadata::TypeName::IUnknown)) => methods.combine(&quote! { base__: windows_core::IUnknown_Vtbl::new::<Identity, OFFSET>(), }),
