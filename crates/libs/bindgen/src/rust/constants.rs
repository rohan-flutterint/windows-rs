...
pub fn writer(writer: &Writer, def: metadata::Field) -> TokenStream {
    ...
    if let Some(constant) = def.constant() {
        ...
        let guid = writer.type_name(&metadata::Type::TypeName(metadata::TypeName::GUID));
        quote! {
...
fn is_signed_error(ty: &metadata::Type) -> bool {
    match ty {
        metadata::Type::TypeName(metadata::TypeName::HResult) => true,
...
fn type_has_replacement(ty: &metadata::Type) -> bool {
    match ty {
        metadata::Type::TypeName(metadata::TypeName::HResult) | metadata::Type::PCSTR | metadata::Type::PCWSTR => true,
...
fn type_underlying_type(ty: &metadata::Type) -> metadata::Type {
    match ty {
        ...
        metadata::Type::TypeName(metadata::TypeName::HResult) => metadata::Type::I32,
        _ => ty.clone(),
    }
}
