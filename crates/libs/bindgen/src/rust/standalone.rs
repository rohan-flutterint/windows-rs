...
pub fn standalone_imp(writer: &Writer) -> String {
    ...
    for ty in types {
        match ty {
            metadata::Type::TypeRef(metadata::TypeName::HResult) if writer.sys => sorted.insert("HRESULT", quote! { pub type HRESULT = i32; }),
...
fn type_collect_standalone(writer: &Writer, ty: &metadata::Type, set: &mut std::collections::BTreeSet<metadata::Type>) {
    ...
    if writer.vtbl {
        match ty {
            metadata::Type::TypeRef(metadata::TypeName::IUnknown) => {
                ...
                set.insert(metadata::Type::TypeRef(metadata::TypeName::HResult));
            }
    ...
    if !type_name.namespace().is_empty() {
        for row in def.reader().get_type_def(&type_name) {
            if def != row {
