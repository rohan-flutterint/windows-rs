...
pub fn standalone_imp(writer: &Writer) -> String {
    ...
    for ty in types {
        match ty {
            ...
            metadata::Type::PSTR if writer.std => sorted.insert("PSTR", quote! { pub type PSTR = *mut u8; }),
            metadata::Type::PWSTR if writer.std => sorted.insert("PWSTR", quote! { pub type PWSTR = *mut u16; }),
            metadata::Type::PCSTR if writer.std => sorted.insert("PCSTR", quote! { pub type PCSTR = *const u8; }),
            metadata::Type::PCWSTR if writer.std => sorted.insert("PCWSTR", quote! { pub type PCWSTR = *const u16; }),
            metadata::Type::BSTR if writer.std => sorted.insert("BSTR", quote! { pub type BSTR = *const u16; }),
